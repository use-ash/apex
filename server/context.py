...
_CALIBRATION_PRIMER_DEFAULT_TEXT = (
    ...
    "- Uncertainty (ICD-203) on major/non-trivial judgments only — not every "
    "sentence. Likelihood and confidence are SEPARATE; never put both in the "
    "same sentence.\n"
    "  Likelihood — use one row only (do not mix rows): almost no chance "
    "(1–5%) | very unlikely (5–20%) | unlikely (20–45%) | roughly even chance "
    "(45–55%) | likely (55–80%) | very likely (80–95%) | almost certain "
    "(95–99%). Alternate row: remote | highly improbable | improbable | "
    "roughly even odds | probable | highly probable | nearly certain.\n"
    "  Confidence (high / moderate / low / unknown) rates the *basis* "
    "(source quality, gaps, assumptions, topic knowledge) — not the odds.\n"
    "  On major judgments: name source quality, mark fact vs assumption vs "
    "judgment, and note one live alternative if uncertainty is material.\n"
...
def _build_recovery_block(chat_id: str, summary: str, session_type: str = "task") -> str:
    ...
    self_orient = (
        ...
        "\n"
        "The API is HTTPS + mTLS — you MUST pass the client cert or the "
        "connection fails (curl exit 52 / empty reply). Cert is on disk:\n"
        "  CERT=~/.openclaw/apex/state/ssl/client.crt\n"
        "  KEY=~/.openclaw/apex/state/ssl/client.key\n"
        "\n"
        "Topic search (preferred — chat-scoped, returns snippets):\n"
        f"  curl -s --cert ~/.openclaw/apex/state/ssl/client.crt --key ~/.openclaw/apex/state/ssl/client.key -k 'https://127.0.0.1:8300/api/chats/{chat_id}/messages?q=<keyword>&limit=20'\n"
