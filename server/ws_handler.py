...
async def _handle_send_action(websocket: WebSocket, data: dict) -> None:
    ...
    try:
        ...
        if is_group_chat and group_agent and response_text:
            premium_relay = (
                _ws_premium.get_agent_relay_actions(
                    chat_id, response_text, group_agent, mention_chain, mention_depth,
                )
                if _ws_premium
                else None
            )
            relay_plan = _build_group_relay_plan(
                chat_id,
                response_text,
                group_agent,
                mention_chain,
                ...
                premium_relay=premium_relay,
            )
            relay_members = relay_plan.relay_members
            relay = relay_plan.relay
            if relay_plan.broadcast_mention_present:
            ...
            if relay_plan.missing_relay_mentions:
                ...
                log(
                    ...
                    f"missing={relay_plan.missing_relay_mentions!r} chat={chat_id[:8]}"
                )
                await _safe_ws_send_json(
                    original_ws,
                ...
                if (
                    ...
                    log(
                        ...
                        f"missing=@{relay_plan.missing_relay_mentions[0]} chat={chat_id[:8]}"
                    )
                    feedback_task = asyncio.create_task(
                        _handle_send_action(original_ws, feedback_data)
                    )
