        ...
        ScrollViewReader { proxy in
            ScrollView {
                VStack(spacing: 8) {
                    ChatMessageList(
                        ...
                        onReact: updateReaction(_:for:),
                        onReply: setReplyTarget(_:)
                    )
            ...
            .safeAreaInset(edge: .bottom) {
                ChatComposeBar(
                    ...
                    onArmStreamingTimeout: armStreamingTimeout,
                    onScrollToBottom: { scrollToBottom(proxy: proxy) }
                )
            }
...
private struct ChatMessageList: View {
    ...
    let highlightedMessageIDs: Set<String>
    let reactions: [String: String]
    let fontScale: CGFloat
    let onReact: (String?, Message) -> Void
...
private struct ChatComposeBar: View {
    ...
    let onArmStreamingTimeout: () -> Void
    ...
    var body: some View {
        ...
        .onChange(of: replyingToMessage) { _, message in
