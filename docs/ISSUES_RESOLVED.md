...
**Root cause:** When the legacy search backend was replaced with Gemini embedding search, `run_memory_search()` was updated to return `{"score", "ref"}` only. Downstream code (`first_line_of_content()`, auto-load content) still expected a `"lines"` key from the old `parse_results()` format.
...
### #F3 — [ui] Guardrail alerts: show outcome (Approved/Rejected), not just "Acknowledged"
**When:** 2026-03-30 &bull; **Component:** alerts / guardrails / ui
**Reporter:** Designer review

**Problem:** Guardrail alerts currently show "✓ Acknowledged" regardless of whether the user approved or rejected the blocked action. "Acknowledged" is a read receipt — it doesn't convey the decision that was made.

**Three distinct outcomes need distinct UI:**
| Outcome | Meaning | Icon | When |
|---------|---------|------|------|
| ✅ Approved | User allowed the blocked action to proceed | Green check | Guardrail approved |
| 🚫 Rejected | User denied the blocked action | Red slash | Guardrail rejected |
| 👁️ Acknowledged | User saw an info-only alert (no decision) | Gray eye | Info/test alerts |

**Changes required:**

1. **Data model** — Guardrail alerts need an `outcome` field: `approved`, `rejected`, or `acknowledged`. Currently only stores `acked: bool`.

2. **Detail pane (quicklook)** — Add an OUTCOME section prominently between title and details. Color-coded: green for approved, red for rejected, gray for ack-only. Show what actually happened: "Edit was allowed" / "Edit was blocked."

3. **List view** — Alert icon in the list should reflect the outcome, not just the category. Currently every guardrail gets the same ⚠️ triangle. After fix: ✅/🚫/👁️ so you can scan your decision history at a glance.

4. **Both platforms** — Web app alerts dropdown + iOS alert inbox.

**OSS note:** Guardrail audit trail is a security feature. Knowing whether a protected-file edit was approved or rejected is essential for compliance and incident review.
