...
3. Ensure DEBUG log level is never enabled in production (`LOG_LEVEL=WARNING` default in `launch_dana.sh`).
...
4. Update `launch_dana.sh` to explicitly set `APEX_HOST` for the known deployment topology.
