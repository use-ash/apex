...
├── Model routing layer
│   ├── Claude (Agent SDK — persistent sessions, tools)
│   ├── Codex (CLI wrapper — background delegation)
│   ├── Grok (xAI API — web search, 2M context)
...
├── Skills engine
│   ├── Server-side dispatch (/recall, /codex, /grok, ...)
│   ├── Thinking skills (agent follows instructions)
│   ├── Metrics + feedback tracking
│   └── User-extensible (drop in a SKILL.md)
├── Alert system (WebSocket + Telegram + REST)
├── SQLite database (chats, messages, alerts)
├── TLS + mTLS (client certificate auth)
└── Embedded HTML/CSS/JS chat UI (PWA)
```

No frameworks. No npm. No build step. The entire frontend is embedded in the Python file. One file to deploy, audit, and understand.
...
**Apex Pro** is a native iOS app that connects to your self-hosted server. Available as a subscription.
...
The server and webapp are free. The iOS app is the paid product.
...
The webapp works in mobile browsers. For a native experience, Apex Pro (iOS) is available as a subscription.
