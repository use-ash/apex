...
# -------------------------------------------
echo "[2] No Dana-specific content in server/, scripts/, docs/"

DANA_HITS=$(grep -rn -i 'dana' server/ scripts/ docs/ --include='*.py' --include='*.sh' --include='*.md' 2>/dev/null | grep -v '__pycache__\|launch_dana\|CHANGELOG\|test_fresh_install' | wc -l)
if [ "$DANA_HITS" -eq 0 ]; then
    pass "No 'dana' references in shipped code/docs"
else
    fail "Found $DANA_HITS 'dana' references"
    grep -rn -i 'dana' server/ scripts/ docs/ --include='*.py' --include='*.sh' --include='*.md' 2>/dev/null | grep -v '__pycache__\|launch_dana\|CHANGELOG\|test_fresh_install' | head -5
fi

ICLOUD_HITS=$(grep -rn 'iCloud\|Mobile Documents\|CloudDocs' server/ --include='*.py' 2>/dev/null | grep -v '__pycache__' | wc -l)
if [ "$ICLOUD_HITS" -eq 0 ]; then
    pass "No iCloud/personal path references in server code"
else
    fail "Found $ICLOUD_HITS iCloud path references in server/"
    grep -rn 'iCloud\|Mobile Documents\|CloudDocs' server/ --include='*.py' 2>/dev/null | grep -v '__pycache__' | head -5
fi
