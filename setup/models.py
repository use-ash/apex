...
def install_claude() -> bool:
    ...
    if npm:
        try:
            ...
            print("Claude Code installed. Run `claude` once to authenticate.")
            return True
        except (subprocess.SubprocessError, OSError):
            pass
    ...
    if brew:
        try:
            ...
            print("Claude Code installed via Homebrew. Run `claude` once to authenticate.")
            return True
        except (subprocess.SubprocessError, OSError):
            pass

    print("Could not install Claude Code. Install npm or Homebrew and retry.")
    return False
...
def install_codex() -> bool:
    ...
    npm = shutil.which("npm")
    if not npm:
        print("npm not found. Install Node.js first, then retry.")
        return False
    try:
        subprocess.run(
        ...
        print("Codex installed. Run `codex` once to authenticate.")
        return True
    except (subprocess.SubprocessError, OSError) as e:
        print(f"Codex install failed: {e}")
        return False
...
def run_models_setup(apex_root: Path, env_dir: Path) -> dict:
    ...
    if not hw.is_apple_silicon or ask_yes_no("Set up Ollama for local models?"):
        ...
        if recs and summary["ollama"].get("running"):
            rec = next((m for m in recs if m["runtime"] == "ollama"), None)
            if rec and rec["name"] not in summary["ollama"].get("models", []):
                if ask_yes_no(f"Pull {rec['name']}?"):
                    if pull_ollama_model(rec["name"]):
                        summary["local_models"].append(rec["name"])
...
def _print_health_table(summary: dict) -> None:
    ...
    for label, status in rows:
        check = "+" if "installed" in status or "configured" in status or "running" in status else "-"
        print(f"  [{check}] {label:<{max_label}}  {status}")
