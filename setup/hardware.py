...
def recommend_models(hw: HardwareInfo) -> list[dict]:
    ...
    The top-tier models (matching the highest RAM bracket) are marked as
    recommended.  Lower-tier models that still fit in RAM and disk are
    included as alternatives so users can choose a faster/smaller option.
    """
    ...
    for min_ram, ollama, mlx_name, size_gb, desc in _MODEL_TABLE:
        if min_ram > hw.ram_gb or size_gb > hw.disk_available_gb:
            continue
        name = f"mlx-community/{mlx_name}" if mlx else ollama
        runtime = "mlx" if mlx else "ollama"
        is_top = min_ram == tier and not seen_top
        if is_top:
            seen_top = True
        candidates.append({"name": name, "ollama_name": ollama, "size_gb": size_gb,
                           "description": desc, "recommended": is_top,
                           "runtime": runtime, "tier": "recommended" if min_ram == tier else "alternative"})
    return candidates
